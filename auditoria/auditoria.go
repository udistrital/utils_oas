...
// the orm.LogFunc hook can ever be request-scoped, and beego v1 has no ORM
// filter chain — there is no hook that sees both the query and the request.
//
// Fixing it means wrapping the database/sql driver with one implementing
// driver.QueryerContext and driver.ExecerContext, which receive both the query
// string and the context. That requires changes in the database package.
